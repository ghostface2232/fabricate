...
float carbonFiberTone(float alongPhase, float acrossCoord, float mask, float gapWidth) {
  ...
  float strand = clamp(primary * 0.94 + secondary * 0.46 + tertiary * 0.24, 0.0, 1.0);
...
void main() {
  ...
  if (carbonFactor > 0.5) {
    ...
    vec3 warpTowColor = u_color1 * 0.965;
    vec3 weftTowColor = u_color1 * 1.0;
  ...
  if (carbonFactor > 0.5) {
    ...
    fiberTone = carbonFiberTone(topPhase, topAcross, pow(clamp(topTowMask, 0.0, 1.0), 0.92), u_gapWidth);
  }
  yarnColor *= 1.0 + fiberTone * mix(0.038, 0.28, carbonFactor);
  if (carbonFactor > 0.5) {
    float topTowMask = mix(weftTopShape, warpTopShape, overFactor);
    float centerBand = pow(clamp(topTowMask, 0.0, 1.0), 1.15);
    float strandMix = clamp(fiberTone * 1.3, 0.0, 1.0);
    yarnColor *= mix(0.92, 1.36, strandMix) * mix(0.92, 1.08, centerBand);
  }
